...
public class CustomerController {
    ...
    public ResponseEntity<?> getCustomerById(@PathVariable("id") Integer id){
        return customerService.getCustomerById(id).isPresent() ? new ResponseEntity<>(new CustomResponse(customerService.getCustomerById(id).get()), HttpStatus.CREATED) : new ResponseEntity<>(new CustomResponse(new Customer()), HttpStatus.BAD_REQUEST);
    }
